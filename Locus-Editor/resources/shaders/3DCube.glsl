...
{
	PointLights u_PointLights[MAX_POINT_LIGHTS];
};
...
void main()
{
	...
	for (int i = 0; i < MAX_POINT_LIGHTS; i++) 
	{
		if (u_PointLights[i].Enabled)
		{
			...
			vec3 radiance = u_PointLights[i].Color.xyz * u_PointLights[i].Intensity * attenuation;
			...
			Lo += (kD * v_Albedo.xyz / PI + specular) * radiance * NdotL;  // note that we already multiplied the BRDF by the Fresnel (kS) so we won't multiply by kS again
		}
	}
