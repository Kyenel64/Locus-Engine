...
namespace SideA
{
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			SIDEA_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified!");
	...
	void OpenGLShader::CompileShader(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		SIDEA_CORE_ASSERT(shaderSources.size() <= 2, "Only supports 2 shaders for now");
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				SIDEA_CORE_ERROR("{0}", infoLog.data());
				SIDEA_CORE_ASSERT(false, "Shader compilation failure!");
				break;
			}
		...
		if (isLinked == GL_FALSE)
		{
			...
			SIDEA_CORE_ERROR("{0}", infoLog.data());
			SIDEA_CORE_ASSERT(false, "Shader link failure!");
			return;
		}
